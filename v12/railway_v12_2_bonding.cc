...
class BondingHeader : public Header
{
  public:
    ...
    {
        static TypeId tid = TypeId("RailwayBondingHeaderV122")
            .SetParent<Header>()
...
class BondingReceiver : public Application
{
  public:
    ...
    uint64_t EstimatedLostPackets() const { return m_estimatedLostPackets; }

  private:
    static constexpr uint32_t kGapTimeoutMs = 60;
    ...
    void ArmGapTimer(uint64_t sequence)
    {
        m_gapSequence = sequence;
        ...
        m_gapEvent = Simulator::Schedule(
            ...
            &BondingReceiver::ExpireGap,
            this,
            sequence);
    }

    void ExpireGap(uint64_t sequence)
    {
        if (sequence != m_nextExpected || m_reorderBuffer.empty())
        {
            return;
        }
        ...
        {
            // Treat the missing sequence range as lost so later packets can
            // be delivered rather than blocking the entire stream forever.
            m_estimatedLostPackets += first->first - m_nextExpected;
    ...
    void HandleRead(Ptr<Socket> socket)
    {
        while (Ptr<Packet> packet = socket->Recv())
        {
            ...
            m_seenSequences.emplace(sequence);
    ...
    uint64_t m_estimatedLostPackets = 0;
    EventId m_gapEvent;
};
...
int main(int argc, char* argv[])
{
    ...
    const double offeredMbps =
        static_cast<double>(packetBytes) * 8.0 * 1000000.0 /
        static_cast<double>(intervalUs) / 1000000.0;
    ...
    std::cout << "\n====================================================\n";
    std::cout << " Railway Multi-Connectivity Gateway V12.2\n";
    std::cout << " Saturating Packet-Level Bonding + Gap Recovery\n";
    std::cout << "====================================================\n\n";
