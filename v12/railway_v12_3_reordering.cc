...
class BondingHeader : public Header
{
  public:
    ...
    {
        static TypeId tid = TypeId("RailwayBondingHeaderV123")
            .SetParent<Header>()
...
class LatencyAwareReceiver : public Application
{
  public:
    void AddSocket(Ptr<Socket> socket)
    {
        m_sockets.push_back(socket);
        socket->SetRecvCallback(MakeCallback(&LatencyAwareReceiver::HandleRead, this));
    }
    ...
    uint64_t DeclaredLostPackets() const { return m_declaredLostPackets; }
    uint64_t MaxBufferPackets() const { return m_maxBufferPackets; }

  private:
    static constexpr uint32_t kReorderTimeoutMs = 120;
    ...
    void ArmGapTimer()
    {
        ...
        m_gapEvent = Simulator::Schedule(
            MilliSeconds(kReorderTimeoutMs),
            &LatencyAwareReceiver::ExpireGap,
            this);
    }
    ...
    void ExpireGap()
    {
        ...
        {
            const uint64_t gap = first->first - m_nextExpected;
            m_declaredLostPackets += gap;
            ++m_gapTimeouts;
    ...
    void HandleRead(Ptr<Socket> socket)
    {
        while (Ptr<Packet> packet = socket->Recv())
        {
            ...
            if (m_seenSequences.find(sequence) != m_seenSequences.end())
            {
                ++m_lateOrDuplicate;
                continue;
            }

            m_seenSequences.emplace(sequence, true);
            ++m_uniquePackets;
            m_uniqueBytes += payloadBytes;
            ...
            m_maxBufferPackets = std::max<uint64_t>(
                m_maxBufferPackets,
                m_reorderBuffer.size());
    ...
    std::map<uint64_t, bool> m_seenSequences;
    ...
    uint64_t m_declaredLostPackets = 0;
    uint64_t m_maxBufferPackets = 0;
    EventId m_gapEvent;
};
...
int main(int argc, char* argv[])
{
    ...
    auto rxApp = CreateObject<LatencyAwareReceiver>();
    ...
    const double offeredMbps =
        ...
        (static_cast<double>(intervalUs) * 1.0e-6) /
        1.0e6;

    const double receivedGoodputMbps =
        static_cast<double>(rxApp->UniqueBytes()) * 8.0 /
        measuredSeconds / 1.0e6;

    const double inOrderGoodputMbps =
        static_cast<double>(rxApp->InOrderBytes()) * 8.0 /
        measuredSeconds / 1.0e6;

    const double reorderRate = rxApp->ReceivedPackets() == 0
        ? 0.0
        : 100.0 * static_cast<double>(rxApp->ReorderedPackets()) /
          static_cast<double>(rxApp->ReceivedPackets());

    std::cout << "\n====================================================\n";
    std::cout << " Railway Multi-Connectivity Gateway V12.3\n";
    ...
    std::cout << "Reorder timeout: 120 ms\n";
    std::cout << "Simulation: " << simulationSeconds << " seconds\n\n";

    std::cout << "----------- V12.3 RESULTS -----------\n";
    ...
    std::cout << "Declared lost packets  : " << rxApp->DeclaredLostPackets() << "\n";
    std::cout << "Max reorder buffer     : " << rxApp->MaxBufferPackets() << " packets\n";
    std::cout << "Unique received goodput: " << receivedGoodputMbps << " Mbps\n";
    std::cout << "In-order goodput       : " << inOrderGoodputMbps << " Mbps\n";
    std::cout << "-------------------------------------\n\n";
